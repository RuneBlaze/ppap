...
export class CardManager {
  ...
  private calculateCardPosition(zoneName: string, index: number): { x: number, y: number } {
    switch (zoneName) {
      case 'hand':
  ...
  private calculatePlayPosition(index: number): { x: number, y: number } {
    ...
    const spacing = Math.min(60, zone.bounds.width / Math.max(1, zone.cards.length))
    const startX = zone.bounds.x + (zone.bounds.width - (zone.cards.length * spacing)) / 2
