...
export class CardSprite extends Phaser.GameObjects.Container {
	...
	private initializeShader(): void {
		...
		if (renderer.type === Phaser.WEBGL) {
			...
			if (pipelineManager && !pipelineManager.get("Card3DHover")) {
				pipelineManager.add(
					"Card3DHover",
					new Card3DHoverShader(this.scene.game),
				);
			}
	...
	private setupMouseTracking(): void {
		...
		this.scene.input.on("pointermove", (pointer: Phaser.Input.Pointer) => {
			...
			if (deltaTime > 0) {
				this.velocity.x =
					(currentMousePos.x - this.lastMousePos.x) / (deltaTime * 4000);
				this.velocity.y =
					(currentMousePos.y - this.lastMousePos.y) / (deltaTime * 4000);
	...
	private moveToResolvingPosition(): void {
		...
		const centerX =
			this.scene.cameras.main.width / 2 - CardSprite.CARD_WIDTH / 2;
		const centerY =
			this.scene.cameras.main.height / 2 - CardSprite.CARD_HEIGHT / 2;
	...
	private moveOffScreen(): void {
		...
		const offScreenY =
			this.scene.cameras.main.height - CardSprite.CARD_HEIGHT - 10;
		...
		if (this.hoverShader) {
			...
			if (hovering) {
				...
				if (pointer) {
					...
					this.shaderUniforms.mouseScreenPos = [
						mouseRelative.x,
						mouseRelative.y,
					];
				}
			...
			try {
				...
				this.hoverShader.set2f(
					"cardSize",
					this.shaderUniforms.cardSize[0],
					this.shaderUniforms.cardSize[1],
				);
	...
	update(_time: number, delta: number): void {
		...
		if (this.isDragging && pointer) {
			...
			this.currentRotation +=
				Math.sin(this.TIME + 1321 + this.randomOffset) * (0.003625 / 2);

			// Add subtle oscillating position movement
			this.x +=
				Math.cos(this.TIME + 180 + 1321 + this.randomOffset) * (0.875 / 2);
			this.y +=
				Math.sin(this.TIME + 360 + 1231 + this.randomOffset) * (0.875 / 2);

			// Handle hover effects and shaders
			if (
				this.isCursorTouching() &&
				this.enabled &&
				this.cardState === CardState.ACTIVE
			) {
	...
	private renderSerifText(
		...
		const adjustedChar =
			compression <= 0 ? character.toUpperCase() : character.toLowerCase();
