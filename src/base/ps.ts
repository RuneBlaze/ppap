...
import { PsPalette, PsPaletteArray } from "./ps_palette";
...
function blt(scene: Phaser.Scene, n: number, x: number, y: number) {
...
class Emitter {
  ...
  private spawn(n: number) {
    if (this.particles.length >= this.config.max_p) return;
