...
import { FocusableList, FocusableGrid, FocusableMenu } from '../ui/state/FocusableWrappers';
...
export class ShopScene extends BaseScene {
  ...
  private createShopMenuButton() {
    ...
    this.shopMenuFocusToken = this.focusManager!.register('shopMenuButton', focusableButton);
  }
  ...
  private initializeFocusManager() {
    ...
    this.focusManager.on('focusChanged', (event: any) => {
      console.log(`[${event.layer}] Focus changed: ${event.oldName || 'none'} → ${event.newName || 'none'}`);
    });
    
    // Listen for layer changes
    this.focusManager.on('layerChanged', (event: any) => {
      console.log(`Layer ${event.action}: ${event.layerName} (depth: ${event.depth})`);
    });
  }
  ...
  private createScrollableListDemo() {
    ...
    this.listFocusToken = this.focusManager!.register('itemList', this.focusableList);
  }
  ...
  private createGridNavigationDemo() {
    ...
    this.gridNavigation.on('changed', (index: number, oldIndex: number, _row: number, _col: number) => {
    ...
    this.gridFocusToken = this.focusManager!.register('gridNavigation', this.focusableGrid);
  }
  ...
  private showMainMenu() {
    if (this.mainMenu) {
      ...
      if (this.focusableMainMenu) {
        this.focusManager?.focus((this.focusableMainMenu.getMenu() as any).focusToken);
      }
      return;
    }
    ...
    const token = this.focusManager!.register('mainMenu', this.focusableMainMenu, 'shopMenu');
  ...
  private showBuyMenu() {
    ...
    const token = this.focusManager!.register('buyMenu', this.focusableBuyMenu, 'buyMenu');
  ...
  private showSellMenu() {
    ...
    const token = this.focusManager!.register('sellMenu', this.focusableSellMenu, 'sellMenu');
  ...
  private showGossipMenu() {
    ...
    const token = this.focusManager!.register('gossipMenu', this.focusableGossipMenu, 'gossipMenu');
