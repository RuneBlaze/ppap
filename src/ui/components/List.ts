...
export class List extends Container {
  ...
  constructor(scene: Phaser.Scene, options: ListOptions) {
    ...
    const actualHeight = options.height || 
      Math.min(options.items.length, this.maxVisibleItems) * LINE_HEIGHT + PADDING * 2;
  ...
  private updateDisplay() {
    ...
    const endIndex = Math.min(startIndex + this.maxVisibleItems, this.options.items.length);
