...
export class Gauge extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, private options: GaugeOptions) {
  ...
  private redraw() {
    ...
    // Draw background arc
    this.graphics.lineStyle(thickness, Phaser.Display.Color.HexStringToColor(backgroundColor).color);
    ...
    // Draw fill arc
    if (fillPercentage > 0) {
      this.graphics.lineStyle(thickness, Phaser.Display.Color.HexStringToColor(fillColor).color);
      this.graphics.beginPath();
    ...
    // Draw border
    if (borderColor) {
      this.graphics.lineStyle(1, Phaser.Display.Color.HexStringToColor(borderColor).color);
      this.graphics.beginPath();
