...
export class Slider extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, private options: SliderOptions) {
    super(scene, options.x, options.y);

    this.currentValue = Phaser.Math.Clamp(options.value, options.min, options.max);
  ...
  private handlePointerDown = (pointer: Phaser.Input.Pointer) => {
    ...
    const isHorizontal = (this.options.orientation ?? 'horizontal') === 'horizontal';
  ...
  private getLocalCoordinates(worldX: number, worldY: number): { x: number; y: number } {
  ...
  public setValue(value: number, callCallback: boolean = true) {
    const clampedValue = Phaser.Math.Clamp(value, this.options.min, this.options.max);
