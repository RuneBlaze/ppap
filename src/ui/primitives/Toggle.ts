...
export class Toggle extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, private options: ToggleOptions) {
  ...
  private drawSwitchIndicator(width: number, height: number) {
    ...
    this.indicator.fillStyle(Phaser.Display.Color.HexStringToColor(Palette.WHITE).color);
    this.indicator.fillRoundedRect(knobX, knobY, knobSize, knobSize, knobSize / 4);
  }

  private drawCheckboxIndicator(width: number, height: number) {
    if (this.isOn) {
      this.indicator.lineStyle(2, Phaser.Display.Color.HexStringToColor(Palette.WHITE).color);
  ...
  private drawButtonIndicator(width: number, height: number) {
    ...
    this.indicator.clear();
    this.indicator.fillStyle(Phaser.Display.Color.HexStringToColor(color).color);
    this.indicator.fillCircle(width / 2, height / 2, Math.min(width, height) / 3);
  }

  private setupInteractivity(width: number, height: number) {
    this.setSize(width, height);
    this.setInteractive({ useHandCursor: true })
      .on('pointerup', this.handleToggle);
  }
  ...
  private updateVisualState(animate: boolean = true) {
    ...
    if (this.label && this.options.labels) {
      this.label.setText(this.isOn ? this.options.labels.on : this.options.labels.off);
    }
  ...
  private animateSwitchTransition() {
    ...
    this.switchTween = this.scene.tweens.add({
      ...
      onUpdate: (tween) => {
        ...
        this.indicator.clear();
        this.indicator.fillStyle(Phaser.Display.Color.HexStringToColor(Palette.WHITE).color);
        this.indicator.fillRoundedRect(tweenedObject.x, 2, knobSize, knobSize, knobSize / 4);
      },
      onComplete: () => {
