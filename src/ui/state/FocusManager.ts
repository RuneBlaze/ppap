...
export class FocusManager extends Phaser.Events.EventEmitter {
  ...
  register(name: string, component: Focusable, layerName?: string): FocusToken {
    ...
    const targetLayer = layerName ? this.getOrCreateLayer(layerName) : this.getCurrentLayer();
  ...
  focusNext(): boolean {
    ...
    const currentIndex = currentLayer.focusOrder.indexOf(currentLayer.currentFocus);
  ...
  focusPrevious(): boolean {
    ...
    const currentIndex = currentLayer.focusOrder.indexOf(currentLayer.currentFocus);
    const prevIndex = currentIndex === 0 ? currentLayer.focusOrder.length - 1 : currentIndex - 1;
   ...
   */
  getDebugInfo(): { layers: Array<{ name: string; focusOrder: string[]; currentFocus: string | null }> } {
    return {
      layers: this.layerStack.map(layer => ({
        ...
        currentFocus: layer.currentFocus ? this.registrations.get(layer.currentFocus)?.name || null : null
      }))
    };
  }
